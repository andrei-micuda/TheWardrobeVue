    ...
    <a-row v-if="items" class="w-11/12 md:w-3/5 mx-auto">
      <a-col :span="24" :md="{ span: 12 }" class="space-y-6 mb-6 md:mb-0">
        ...
        <DeliveryAddressRadio
          @deliveryAddressChange="handleDeliveryAddressChange"
        />
      </a-col>
      <a-col :span="24" :md="{ span: 12 }">
        <div class="bg-gray-800 md:ml-10 p-4 rounded text-lg space-y-6">
            ...
            <div class="flex items-center space-x-2">
              ...
              >
                <p id="SellerRating" class="text-sm text-center">
                  {{ sellerRating ? sellerRating.toFixed(2) : "N/A" }}
                </p>
          ...
          <a-button type="primary" size="large" @click="placeOrder"
  ...
  methods: {
    placeOrder() {
      if (!this.deliveryAddress) {
        notifier.error(
          "Please select a delivery address in order to place the order."
        );
        return;
      }

      api
        .post(`/public/api/${store.state.id}/order`, {
    ...
    fetchItems() {
      api
        .get(`/public/api/${store.state.id}/cart`, {
