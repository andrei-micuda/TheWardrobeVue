    ...
    <div class="bg-gray-800 xl:w-1/4 sm:w-96 w-11/12 rounded">
      <div class="my-12 mx-6" v-if="isSent">
        <Icon
          ...
          class="w-24 h-24 text-green-400 mx-auto mb-6"
        />
        <h1 class="text-2xl mb-2 text-gray-100 text-center">
          Reset email sent!
        </h1>
        <p class="text-lg text-center">
          Please check your inbox and follow the instructions in order reset
          your password.
        </p>
      </div>
      <div v-else>
        ...
        <p class="mx-4">
          Please enter your account email below in order to reset your password.
        </p>
        <a-form
          :form="form"
          @submit="handleSubmit"
    ...
    handleSubmit(e) {
      ...
      this.form.validateFields(async (err, values) => {
        if (!err) {
          ...
          api
            .post("/public/api/account/forgot-password", {
              email: values.email,
            })
            .then(() => {
              this.isSent = true;
